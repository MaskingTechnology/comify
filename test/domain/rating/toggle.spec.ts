...
describe('domain/post/toggleRating', () =>
{
    ...
    {
        const isRated = await toggle(REQUESTERS.CREATOR1, TENANTS.default, VALUES.IDS.POST_UNRATED);
        expect(isRated).toBeTruthy();
    });

    it('should remove a rating', async () =>
    {
        const isRated = await toggle(REQUESTERS.CREATOR1, TENANTS.default, VALUES.IDS.POST_RATED);

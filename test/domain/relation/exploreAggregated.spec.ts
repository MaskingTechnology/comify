...
import { SortOrderEnum } from '^/domain/relation/definitions';
...
describe('domain/relation/exploreAggregated', () =>
{
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, SortOrderEnum.POPULAR, undefined, VALUES.RANGE);
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, SortOrderEnum.RECENT, undefined, VALUES.RANGE);
        expect(relations).toHaveLength(3);
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, SortOrderEnum.POPULAR, 'or2', VALUES.RANGE);
        expect(relations).toHaveLength(0);
    });

    it('should find relations based on search full name', async () =>
    {
        const relations = await explore(REQUESTERS.FIRST, SortOrderEnum.POPULAR, 'or 4', VALUES.RANGE);
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, SortOrderEnum.POPULAR, 'creator4', VALUES.RANGE);
        expect(relations).toHaveLength(1);
        expect(relations[0].following?.id).toBe(VALUES.IDS.CREATOR4);
    });
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, SortOrderEnum.POPULAR, 'five', VALUES.RANGE);

...
describe('domain/relation/exploreAggregated', () =>
{
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, TENANTS.default, SortOrders.RECENT, VALUES.RANGE);
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, TENANTS.default, SortOrders.POPULAR, VALUES.RANGE, 'or2');
        expect(relations).toHaveLength(0);
    });

    it('should find relations based on search full name', async () =>
    {
        const relations = await explore(REQUESTERS.FIRST, TENANTS.default, SortOrders.POPULAR, VALUES.RANGE, 'or 4');
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, TENANTS.default, SortOrders.POPULAR, VALUES.RANGE, 'creator4');
        expect(relations).toHaveLength(1);
        expect(relations[0].following?.id).toBe(VALUES.IDS.CREATOR4);
    });
    ...
    {
        const relations = await explore(REQUESTERS.FIRST, TENANTS.default, SortOrders.POPULAR, VALUES.RANGE, 'five');
